public class Seed { // The class initializes the user on interacting with the seed whether to plant, water, harvest and sell the seed/crop
    ...
    private final int produce; // number of producs produced by the crop
    ...
    public Seed(String order){ // a constructor for the class to initializes the interaction of the seed
        ...
        SeedList seedList = new SeedList(); // instantiate the seedList obect with a SeedList class
        ...
        this.produce = seedList.getProduced(this.seedIndex); seedList.getSellingPrice(this.seedIndex); 
    ...
    public void checkCondition(SeedList seedList){ // this method checks the condition of the seed based on number of times watered and fertiized and conditions to become harvestable or withered
    ...
    public double getTotalPrice(){ // computes for the total price from harvesting and selling the seed/crop
        ...
        waterBonus = harvestTotal * 0.2  * (this.numWatered - 1); // the waterbonus gets computed by the harvest total multipled to 0.2 multiplied to one less the number of times watered
        fertilizerBonus = harvestTotal * 0.5 * this.numFertilized; // the fertilizerBonus is computed by the harvestTotal multiplied to 0.5 multiplied to number of times fertlized
        finalHarvestPrice = harvestTotal + waterBonus + fertilizerBonus; // the totalprice is comptued by the sum of harvestTotal, waterBonus and fertilizerBonus
    ...
    public void grow(){ // initializes the growing the crop, decreenting the harvestTime by 1
        this.harvestTime--;
    }
